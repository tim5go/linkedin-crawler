...
module.exports = function profile(url, html, next) {
  ...
  if(education.length > 0){
	  ...
	  for(j = 1; j <= education.length; j++){
		  ed ={};
		  ed.school = $('.school:nth-child('+j+') h4.item-title a').text();
		  ed.cert = $('.school:nth-child('+j+') h5.item-subtitle').text();
		  ed.date = $('.school:nth-child('+j+') .date-range').text();
  ...
  data.links = [];
  var links = $('.profile-card');
  var base_url = 'https://www.linkedin.com/in/';
  if(links.length > 0){
	  for(k = 1; k < links.length; k++){
		  var temp = $('.profile-card:nth-child('+k+') h4.item-title a').attr('href'); 
		  if(temp && temp.startsWith(base_url)){
			  var output = temp.substring(0, temp.lastIndexOf('?'));
			  data.links.push(output);
		  }
	  }
  }
